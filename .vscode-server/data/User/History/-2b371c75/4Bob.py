...
def level2_menu():
    ...
    while animation_running:
        ...
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONDOWN:
                ...
                if pause_button_rect.collidepoint(x, y):
                    ...
                    # Decrease FPS, ensuring it doesn't go below the min FPS
                    fps = max(fps / SPEED_MULTIPLIER, MIN_FPS)
                    print(f"New FPS: {fps}")
        ...
        # Control the frame rate based on the updated fps
        clock.tick(fps)
...
try:
    draw_level1_buttons()  # Draw Level 1 buttons
    reset_timer()  # Start a timer (auto exit after 30 seconds of inactivity)
        ...
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONDOWN:
                ...
                if start_button_rect.collidepoint(x, y):
                    ...
                    level2_menu()  # Switch to Level 2 menu
                ...
                # If clicked outside the buttons, display the coordinates
                else:
                    print(f"Touch at ({x}, {y})")
                    display_touch(x, y)  # Show touch coordinates on the PiTFT

                # Reset timer after each click
                reset_timer()

        # Handle GPIO bailout button (connected to GPIO 17)
    ...
    cleanup_and_exit()  # Catch keyboard interrupt, exit safely