...
try:
    draw_level1_buttons()
    reset_timer()  # Start the timer at the beginning
    while running:
        pitft.update()
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONDOWN:
                ...
                if start_button_rect.collidepoint(x, y):
                    ...
                    level2_menu()  # Switch to Level 2 menu when Start is pressed

                # Quit button press
                elif quit_button_rect.collidepoint(x, y):
                    print("Quit button pressed!")
                    cleanup_and_exit()  # Exit the program when Quit is pressed

                # If user touches outside of buttons, display coordinates on screen
                else:
                    print(f"Touch at ({x}, {y})")
                    display_touch(x, y)  # Display the touch coordinates on the PiTFT screen

                # Reset the timeout timer on every touch
                reset_timer()
