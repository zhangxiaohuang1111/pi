...
speed1 = [3, 3]  # Increased speed for ball1
speed2 = [3, 2]  # Increased speed for ball2
...
def handle_collision(rect1, rect2, speed1, speed2, mass1, mass2):
    ...
    if distance < (rect1.width / 2 + rect2.width / 2):
        ...
        if rel_vel > 0:
            ...
            separation_factor = 1.01  # Slightly separate more than the overlap distance
            rect1.x += separation_factor * overlap * (mass2 / (mass1 + mass2)) * nx
            rect1.y += separation_factor * overlap * (mass2 / (mass1 + mass2)) * ny
            rect2.x -= separation_factor * overlap * (mass1 / (mass1 + mass2)) * nx
            rect2.y -= separation_factor * overlap * (mass1 / (mass1 + mass2)) * ny
