...
def update_display():
    global paused, prev_stateA, prev_stateB
    try:
        while running:
            pitft.update()
            screen.fill(BLACK)
            ...
            for event in pygame.event.get():
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    if 50 <= x <= 110 and 90 <= y <= 150:  # STOP button position and size
                        global prev_stateA, prev_stateB
                        prev_stateA = stateA  # Save current state of Motor A
                        prev_stateB = stateB  # Save current state of Motor B
                        motor_a_stop()  # Stop Motor A
                        motor_b_stop()  # Stop Motor B
                    ...
                    elif 130 <= x <= 190 and 90 <= y <= 150:  # RESUME button position and size
                        if paused:
                            print("RESUME button pressed - Resuming motors")
                            # Resume Motor A to its previous state
                            if prev_stateA == 1:
                                motor_a_forward()
                            elif prev_stateA == 2:
                                motor_a_backward()

                            # Resume Motor B to its previous state
                            if prev_stateB == 1:
                                motor_b_forward()
                            elif prev_stateB == 2:
                                motor_b_backward()
