...
def handle_panic_stop():
    ...
    if not is_panic_stop:
        # Save current state and stop motors
        prev_stateA = stateA
        prev_stateB = stateB
        motor_a_stop()
        motor_b_stop()
    ...
    try:
        while running:
            ...
            for event in pygame.event.get():
                    ...
                    if 130 <= x <= 190 and 90 <= y <= 150:  # STOP/RESUME按钮位置和大小
                        handle_panic_stop()
                    
                    # Check if QUIT button is pressed
                    elif 230 <= x <= 310 and 180 <= y <= 220:  # QUIT按钮位置和大小
                        print("QUIT button pressed")
                        cleanup_and_exit()  # 调用清理和退出函数
...
def handle_buttonA_control(channel):
    global stateA
    if not is_panic_stop:  # 只有在非紧急停止状态下才能控制电机A
        if stateA == 0 or stateA == 2:
            motor_a_forward()  # 启动电机A顺时针
            stateA = 1
        else:
            motor_a_backward()  # 启动电机A逆时针
            stateA = 2

def handle_buttonA_stop(channel):
    global stateA
    if not is_panic_stop:  # 只有在非紧急停止状态下才能停止电机A
        motor_a_stop()
        stateA = 0

def handle_buttonB_control(channel):
    global stateB
    if not is_panic_stop:  # 只有在非紧急停止状态下才能控制电机B
        if stateB == 0 or stateB == 2:
            motor_b_forward()  # 启动电机B顺时针
            stateB = 1
        else:
            motor_b_backward()  # 启动电机B逆时针
            stateB = 2

def handle_buttonB_stop(channel):
    global stateB
    if not is_panic_stop:  # 只有在非紧急停止状态下才能停止电机B
        motor_b_stop()
        stateB = 0
...
# Main loop to ensure smooth operation
try:
    while running:
        time.sleep(0.1)
