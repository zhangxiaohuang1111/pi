...
def handle_buttonA_control(channel):
    global stateA, paused, prev_stateA
    if not paused:
...
# Adjusted display function to include STOP/RESUME toggle and QUIT button interactions
# Adjusted display function to include STOP/RESUME toggle and QUIT button interactions
def update_display():
    global paused, prev_stateA, prev_stateB, stop_resume_state
    try:
        while running:
            pitft.update()
            screen.fill(BLACK)
            ...
            if stop_resume_state == 'STOP':
                pygame.draw.circle(screen, RED, (160, 120), 30)  # Draw STOP button at center
                stop_resume_text = font_small.render("STOP", True, WHITE)
                screen.blit(stop_resume_text, (145, 115))  # Adjust text position
            else:  # If the state is 'RESUME'
                pygame.draw.circle(screen, GREEN, (160, 120), 30)  # Draw RESUME button at center
                stop_resume_text = font_small.render("RESUME", True, WHITE)
                screen.blit(stop_resume_text, (140, 115))  # Adjust text position
            ...
            for event in pygame.event.get():
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    if 130 <= x <= 190 and 90 <= y <= 150:  # Adjust position for STOP/RESUME button
                        if stop_resume_state == 'STOP':
                            # Save current motor states and stop motors
                            prev_stateA = stateA
                            prev_stateB = stateB
                            motor_a_stop()
                            motor_b_stop()
                            paused = True
                            stop_resume_state = 'RESUME'  # Switch to 'RESUME' state
                            print("STOP button pressed - Both motors stopped")
                        elif stop_resume_state == 'RESUME':
                            # Resume motors to previous states
                            if prev_stateA == 1:
                                motor_a_forward()
                            elif prev_stateA == 2:
                                motor_a_backward()
                            ...
                            paused = False
                            stop_resume_state = 'STOP'  # Switch to 'STOP' state
